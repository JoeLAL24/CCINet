...
class Decoder_CCINet(nn.Module):
    def __init__(self):
        super(Decoder_CCINet, self).__init__()
...
class CCI_Net(nn.Module):
    """ Class for extracting activations and
    registering gradients from targetted intermediate layers """
    def __init__(self, mode='train'):
        super(CCI_Net, self).__init__()
        self.gradients = None
        self.backbone = VGG_Backbone()
        self.mode = mode
        self.aug = AugAttentionModule()
        self.fusion = AttLayer(512)
        self.decoder_ccinet = Decoder_CCINet()
    ...
    def _forward(self, x, gt):
        ...
        preds = self.decoder_ccinet(feataug, x4, x3, x2, x1, H, W, edge)
        if self.training:
...
class CCINet(nn.Module):
    def __init__(self, mode='train'):
        super(CCINet, self).__init__()
        set_seed(123)
        self.cci_net = CCI_Net()
        self.mode = mode

    def set_mode(self, mode):
        self.mode = mode
        self.cci_net.set_mode(self.mode)

    def forward(self, x, gt):
        ########## Co-SOD ############
        preds = self.cci_net(x, gt)
        return preds
